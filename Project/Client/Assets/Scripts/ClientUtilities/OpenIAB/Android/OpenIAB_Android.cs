...
namespace OnePF
{
 ...
 : IOpenIAB
#endif
    {
        ...
        public static readonly string STORE_CAFEBAZAAR;
        public static readonly string STORE_MYKET;
        public static readonly string STORE_CHARKHUNEH;
        public static readonly string STORE_IRANAPPS;
        ...
        {
            Debug.Log("[OpenIAB_Android] Constructor");
            if (Application.platform != RuntimePlatform.Android)
            {
                Debug.Log("[OpenIAB_Android] Constructor : Not Android.");
                STORE_GOOGLE = "STORE_GOOGLE";
                STORE_CAFEBAZAAR = "STORE_CAFEBAZAAR";
                STORE_MYKET = "STORE_MYKET";
                STORE_CHARKHUNEH = "STORE_CHARKHUNEH";
                STORE_IRANAPPS = "STORE_IRANAPPS";
            ...
            Debug.Log("[OpenIAB_Android] Find the plugin instance");
            ...
            using (var pluginClass = new AndroidJavaClass("org.onepf.oms.OpenIabHelper"))
            {
                ...
                STORE_CAFEBAZAAR = pluginClass.GetStatic<string>("NAME_CAFEBAZAAR");
                STORE_MYKET = pluginClass.GetStatic<string>("NAME_MYKET");
                STORE_CHARKHUNEH = pluginClass.GetStatic<string>("NAME_CHARKHUNEH");
                STORE_IRANAPPS = pluginClass.GetStatic<string>("NAME_IRANAPPS");
        ...
        private bool IsDevice()
        {
            if (Application.platform == RuntimePlatform.Android)
            {
                //OpenIAB.EventManager.SendMessage("OnBillingNotSupported", "editor mode");//TODO :: Check from here
                Debug.Log("[OpenIAB_Android] IsDevice platform an Android Type? Yes");
                return true;
            }else if (Application.platform == RuntimePlatform.IPhonePlayer)
            {
                Debug.Log("[OpenIAB_Android] IsDevice platform an IOS Type? Yes");
                return true;

            }
            Debug.Log("[OpenIAB_Android] Device platform is not  Android  or Ios Type");
            return false;
        }
        ...
        public void init(Options options)
        {
            if (!IsDevice())
            {
                ...
                Debug.Log("[OpenIAB_Android] Fake init process in the editor. For test purposes");
                OpenIAB.EventManager.SendMessage("OnBillingSupported", "");
                return;
            }

            Debug.Log("[OpenIAB_Android] init Android IAB");
            using (var j_optionsBuilder = new AndroidJavaObject("org.onepf.oms.OpenIabHelper$Options$Builder"))
            {
                ...
                Debug.Log("[OpenIAB_Android] init ,Build options instance");
                ...
                Debug.Log("[OpenIAB_Android] UnityPlugin.initWithOptions(OpenIabHelper.Options options)");
        ...
        {
            Debug.Log("[OpenIAB_Android] init fuction started");
            if (!IsDevice()) return;

            if (storeKeys != null)
            {
                Debug.Log("[OpenIAB_Android] init from Android" + storeKeys.Values);
        ...
        {
            if (!IsDevice()) return;
            Debug.Log("[OpenIAB_Android] mapSku from Android" + sku);
        ...
        {
           Debug.Log("[OpenIAB_Android] queryInventory from Android started");
            if (!IsDevice())
            {
                return;
            }
            ...
            Debug.Log("[OpenIAB_Android] queryInventory from Android Ended");
        }

        public void queryInventory(string[] skus)
        {
            Debug.Log("[OpenIAB_Android] queryInventory from Android array");
        ...
        {
            Debug.Log("[OpenIAB_Android] purchase started");
            if (!IsDevice())
            {
                // Fake purchase in editor mode
                Debug.Log("[OpenIAB_Android] Fake purchase in editor mode");
                OpenIAB.EventManager.SendMessage("OnPurchaseSucceeded", Purchase.CreateFromSku(sku, developerPayload).Serialize());
                return;
            }
            Debug.Log("[OpenIAB_Android] purchase from Android");
        ...
        {
            Debug.Log("[OpenIAB_Android] consumeProduct started");
            if (!IsDevice())
            {
                // Fake consume in editor mode
                Debug.Log("[OpenIAB_Android] Fake consume in editor mode");
                OpenIAB.EventManager.SendMessage("OnConsumePurchaseSucceeded", purchase.Serialize());
                return;
            }
            Debug.Log("[OpenIAB_Android] consumeProduct from Android");
