...
namespace OnePF
{
    ...
    public class OpenIAB
    {
        ...
        public static void init(string storeName, string publicKey)
        {
            Debug.Log("[OpenIAB] creat option in order to start init.");
            var options = new Options();
            options.checkInventoryTimeoutMs = Options.INVENTORY_CHECK_TIMEOUT_MS * 4;
            options.discoveryTimeoutMs = Options.DISCOVER_TIMEOUT_MS * 6;
            options.checkInventory = true;
            options.verifyMode = OptionsVerifyMode.VERIFY_ONLY_KNOWN;
            options.storeKeys = new Dictionary<string, string> { { storeName, publicKey } };
