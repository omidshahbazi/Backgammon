...
namespace RTLTMPro
{
	public class RTLSupport
	{
		...
		protected static readonly Regex PairedTagFixer = new Regex(@"(?<closing></(?<tagName>\p{Ll}+)>)(?<content>(.|\n)+?)(?<opening><\k<tagName>=?(\p{Ll}|\p{N}|-|\+|#)*>)");
		protected static readonly Regex LoneTagFixer = new Regex(@"(?<!</\p{Ll}+>.*)(<\p{Ll}+=?(\p{Ll}|\p{N})+/?>)");
		...
		public virtual string FixRTL(string input)
		{
			...
			//char[] shapeFixedLetters =;

			// Fix flow of the text and put the result in FinalLetters field
			FixLigature(FixGlyphs(PrepareInput(input)));
		...
		{
			return IsRTLInput(input.ToCharArray());
		}
		...
		/// </summary>
		/// <returns><see langword="true" /> if input is RTL. otherwise <see langword="false" /></returns>
		public virtual bool IsRTLInput(char[] chars)
		{
			...
			for (int i = 0; i < chars.Length; ++i)
			{
				...
				{
					if ((/*LocalizationManager.CurrentLanguage.Unicode &&*/ (character == 'X' || character == 'x')))
					{
						if ((i + 1) < chars.Length)
						{
							return char.IsDigit(chars[i + 1]);
						}
					}
				...
				if (/*LocalizationManager.CurrentLanguage.Unicode &&*/ char.IsDigit(character))
					return true;
		...
		protected virtual string FixTags(string input)
		{
			...
			UnityEngine.Debug.Assert(FixTextTags != false, "FixTextTags is false but you have called FixTags method. This method will not work properly when FixTextTags is false");
			...
			MatchCollection tags = PairedTagFixer.Matches(input);
			...
			for (int i = 0; i < tags.Count; ++i)
			{
				...
				int closingLength = match.Groups["closing"].Value.Reverse().ToArray().ArrayToString().Length;
				int contentLength = match.Groups["content"].Value.Length;
				input = input.Insert(match.Index, match.Groups["closing"].Value.Reverse().ToArray().ArrayToString());
				input = input.Insert(match.Index + closingLength, match.Groups["content"].Value);
				input = input.Insert(match.Index + closingLength + contentLength, match.Groups["opening"].Value.Reverse().ToArray().ArrayToString());
			}
			...
			for (int i = 0; i < tags.Count; ++i)
			{
				...
				input = input.Remove(match.Index, match.Length);
				//string opening = ;
				input = input.Insert(match.Index, match.Value.Reverse().ToArray().ArrayToString());
			}
		...
		{
			char[] letters = RemoveTashkeel(input).ToCharArray();
		...
		protected virtual string RemoveTashkeel(string str)
		{
			...
			for (int i = 0; i < letters.Length; i++)
			{
				...
				switch (letters[i])
				{
					case (char)0x064B:
						// Tanween Fatha
						character = (char)0x064B;
						break;
					case (char)0x064C:
						// Tanween Damma
						character = (char)0x064C;
						break;
					case (char)0x064D:
						// Tanween Kasra
						character = (char)0x064D;
						break;
					case (char)0x064E:
						character = (char)0x064E;
						break;
					case (char)0x064F:
						character = (char)0x064F;
						break;
					case (char)0x0650:
						character = (char)0x0650;
						break;
					case (char)0x0651:
						character = (char)0x0651;
						break;
					case (char)0x0652:
						// SUKUN
						character = (char)0x0652;
						break;
					case (char)0x0653:
						// MADDAH ABOVE
						character = (char)0x0653;
						break;
					case (char)0x670:
						// Superscript alef
						character = (char)0x670;
						break;
					default:
						break;
				...
				if (character != ' ')
					TashkeelLocation.Add(new TashkeelLocation(character, i));
			}
		...
		protected virtual void FixLigature(char[] shapeFixedLetters)
		{
			...
			for (int i = shapeFixedLetters.Length - 1; i >= 0; i--)
			{
				bool isInMiddle = i > 0 && i < shapeFixedLetters.Length - 1;
				bool isAtEnd = i == shapeFixedLetters.Length - 1;
				...
				if (char.IsPunctuation(shapeFixedLetters[i]) || char.IsSymbol(shapeFixedLetters[i]))
				{
						...
						if (shapeFixedLetters[i] == '>')
						{
							...
							if (isAtBegining == false)
							{
								for (int j = i - 1; j >= 0; j--)
								{
									...
									//if (shapeFixedLetters[j] == ' ')//Memo: Commented By Memo because actually we have space in tags
									//{
									//	break;
									//}
					...
					if (FixTextTags)
					{
						if (shapeFixedLetters[i] == '<')
						{
							...
							if (isAtEnd == false)
							{
								for (int j = i + 1; j < shapeFixedLetters.Length; j++)
								{
									// Tags do not have space inside
									//if (shapeFixedLetters[j] == ' ') //Memo: Commented By Memo because actually we have space in tags
									//{
							...
							if (ltrText.Count > 0 && valid)
							{
								...
								{
									for (int j = 0; j < ltrText.Count; j++)
										FinalLetters.Add(ltrText[j]); //Memo: Added chars from start because we will reverse it finally
																	  //FinalLetters.Add(ltrText[ltrText.Count - 1 - j]);
								}
				...
				if (isInMiddle)
				{
					...
					bool isAfterSymbol = char.IsSymbol(shapeFixedLetters[i + 1]) || shapeFixedLetters[i + 1] == '"'; // Added By Memo: we have (") as symbol
					bool isBeforeSymbol = char.IsSymbol(shapeFixedLetters[i - 1]) || shapeFixedLetters[i - 1] == '"';// Added By Memo: we have (") as symbol
		...
		protected virtual char[] RestoreTashkeel(char[] letters)
		{
			...
			for (int i = 0; i < letters.Length; ++i)
			{
				...
				{
					tashkeelLocation = TashkeelLocation[j];
					if (tashkeelLocation.Position == letterWithTashkeelTracker)
					{
						lettersWithTashkeel[letterWithTashkeelTracker] = tashkeelLocation.Tashkeel;
						letterWithTashkeelTracker++;
					}
				}
			}
