...
namespace Networking.Server.Data
{
	static class DatabaseGenerator
	{
		...
		private static Table UsersGameTable = new Table("users_game", Collates.UTF8, Engines.InnoDB, new Column("id", DataType.Int, Flags.PrimaryKey | Flags.AutoIncrement), new Column("type", DataType.Int), new Column("table_id", DataType.Int), new Column("white_user_id", DataType.Int), new Column("black_user_id", DataType.Int), new Column("bot_user_info", new DataType("VARCHAR", 500)), new Column("winner_user_id", DataType.Int), new Column("finish_reason", DataType.Int), new Column("start_time", DataType.DateTime), new Column("end_time", DataType.DateTime), new Column("version", DataType.Int), new Column("replay_data", DataType.LongBlob));
